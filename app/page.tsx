    ...
    <div className="transition-transform duration-200 ease-out hover:-translate-y-1 hover:scale-[1.01]">
      ...
      >
        <div className="p-5 flex flex-col gap-3">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusBadge[alert.status] ?? statusBadge.open}`}>
                    {alert.status}
                  </span>
          ...
          {/* Expanded AI panel — cursor-default, no card toggle */}
          {expanded && (
            <div className="bg-zinc-900/70 rounded-xl p-4 flex flex-col gap-3 text-xs text-zinc-300 border border-zinc-800 animate-in fade-in slide-in-from-top-1 duration-200 cursor-default">
              <div>
                ...
                {loadingAI ? (
                  <p className="mt-1.5 leading-5 text-zinc-400 animate-pulse">Analyzing with Gemini...</p>
                ) : aiError ? (
    ...
    async function loadData() {
      try {
        ...
        if (!cancelled && rows && rows.length > 0) {
          const sfAlerts: Alert[] = rows.map((row: any) => ({
            ...
            risk: riskFromSeverity(
              (row.SEVERITY || row.severity || "medium").toLowerCase(),
              row.SUPPORTING_EVENT_COUNT || row.supporting_event_count || 0
            ),
            ...
            status: row.STATUS ? row.STATUS.toLowerCase() : row.status ? row.status.toLowerCase() : "open",
          }));
          setAlerts(sfAlerts);
        ...
        // Snowflake failed, try Mongo next
      }
              ...
              >
                <div className="p-5 flex flex-col gap-1">
                  <span className={`text-3xl font-bold tracking-tight ${s.color}`}>
                    <CountUp from={0} to={s.to} separator="," direction="up" duration={1.5} startCounting={true} />
                    {s.suffix}
                  </span>
        ...
        <section>
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-widest">
              Live Threat Alerts
              {dataSource === "snowflake" && (
                <span className="ml-2 text-sky-400 normal-case font-normal">(from Snowflake)</span>
              )}
              {dataSource === "mongo" && (
                <span className="ml-2 text-emerald-500 normal-case font-normal">(from MongoDB)</span>
              )}
            </h2>
