...
import { analyzeIncident, getIncidents, type MongoIncident } from "@/lib/api";
...
function formatTitle(attack_type: string): string {
  return attack_type
    .replace(/_/g, " ")
...
function riskFromSeverity(severity: string, failedAttempts: number): number {
  ...
  const b = base[severity] ?? 50;
  const bonus = Math.min(failedAttempts, 10);
  return Math.min(b + bonus, 100);
}
...
const fallbackAlerts: Alert[] = [
  ...
  },
  {
    ...
    risk: 77,
    aiExplanation:
      "API endpoint /export/data called 312 times in 3 minutes by a low-privilege service account. Snowflake time-series analysis shows 40x spike above baseline.",
...
const severityConfig: Record<
  string,
  { badge: string; glowColor: string; colors: string[] }
> = {
  critical: {
            ...
            onClick={async () => {
              ...
              if (willExpand && !aiResult && !loadingAI) {
                ...
                try {
                  ...
                  setAiError(
                    err?.message || "Failed to fetch AI analysis",
                  );
                } finally {
                  setLoadingAI(false);
                }
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  <span
                    className={`text-xs font-bold uppercase px-2 py-0.5 rounded-full ${cfg.badge}`}
                  >
                    {alert.severity}
                  </span>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusBadge[alert.status] ?? statusBadge.open}`}
                  >
                    {alert.status}
                  </span>
                ...
                <p
                  className={`text-xs text-zinc-500 ${geistMono.className}`}
                >
                  {alert.source} &middot; {alert.ip}
                </p>
              </div>
          ...
          {expanded && (
            <div className="bg-zinc-900/70 rounded-xl p-4 flex flex-col gap-3 text-xs text-zinc-300 border border-zinc-800 animate-in fade-in slide-in-from-top-1 duration-200 cursor-default">
              <div>
                ...
                {loadingAI ? (
                  <p className="mt-1.5 leading-5 text-zinc-400">
                    Analyzing...
                  </p>
                ) : aiError ? (
              ...
              <div className="border-t border-zinc-800 pt-3">
                <span className="font-semibold text-orange-400 tracking-wide">
                  Root Cause
                </span>
...
export default function Home() {
  ...
  const [dataSource, setDataSource] = useState<"loading" | "mongo" | "fallback">("loading");

  useEffect(() => {
    let cancelled = false;
    getIncidents(50).then((docs) => {
      if (cancelled) return;
      if (docs.length > 0) {
        setAlerts(docs.map(mongoToAlert));
        setDataSource("mongo");
      } else {
    ...
    <div className={`relative min-h-screen text-zinc-100 ${geist.className}`}>
      ...
      <header className="sticky top-0 z-20 bg-zinc-950/60 backdrop-blur-md border-b border-zinc-800/60 px-6 py-4 flex items-center justify-between">
        ...
        <span className="flex items-center gap-1.5 text-xs text-emerald-400 font-medium">
          ...
          {dataSource === "mongo" ? "Live — MongoDB" : dataSource === "loading" ? "Connecting..." : "Demo Mode"}
        </span>
      </header>
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 py-8 flex flex-col gap-10">
            ...
            >
              ...
              >
                <div className="p-5 flex flex-col gap-1">
                  <span
                    className={`text-3xl font-bold tracking-tight ${s.color}`}
                  >
                    <CountUp
                      from={0}
                      to={s.to}
                ...
                >
                  <div className="p-5 flex flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-zinc-600 ml-auto font-light">
                        — {c.role}
                      </span>
                    </div>
                    ...
                    <span
                      className={`text-xs text-zinc-600 mt-0.5 ${geistMono.className}`}
                    >
                      // {c.tag}
                    </span>
                  </div>
        ...
        <p
          className={`text-center text-xs text-zinc-700 pb-4 ${geistMono.className}`}
        >
          AI analysis by Gemini · Data from Snowflake · Memory via MongoDB ·
          Orchestrated by FastAPI
        </p>
      </main>
    </div>
  );
